...
package net.mcreator.tllbutinblocks.world.dimension;
...
import net.mcreator.tllbutinblocks.item.OrganicDimensionItem;
import net.mcreator.tllbutinblocks.block.TooframeblockBlock;
...
public class OrganicDimensionDimension extends TllButInBlocksModElements.ModElement {
	@ObjectHolder("tll_but_in_blocks:organic_dimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("tll_but_in_blocks:organic_dimension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public OrganicDimensionDimension(TllButInBlocksModElements instance) {
		super(instance, 12);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("organic_dimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("tll_but_in_blocks:organic_dimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("tll_but_in_blocks:organic_dimension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("tll_but_in_blocks:organic_dimension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("tll_but_in_blocks:flesh_islands")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("tll_but_in_blocks:brainylands")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("tll_but_in_blocks:blodcelltornadoland")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new OrganicDimensionItem().setRegistryName("organic_dimension"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("organic_dimension_portal");
		}
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS
								.getValue(new ResourceLocation(("tll_but_in_blocks:portal_fleshy"))),
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = OrganicDimensionDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == TooframeblockBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == TooframeblockBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == TooframeblockBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("organic_dimension_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "organic_dimension_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = OrganicDimensionDimension.CustomPortalBlock.createPatternHelper(this.world,
						blockpos);
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? TooframeblockBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, TooframeblockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
	...
	public static class CustomDimension extends Dimension {
		...
		public Vec3d getFogColor(float cangle, float ticks) {
			return new Vec3d(0.349019607843, 0.141176470588, 0.458823529412);
		}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new EndGenerationSettings() {
				public BlockState getDefaultBlock() {
					return FleshBlockBlock.block.getDefaultState();
				}
